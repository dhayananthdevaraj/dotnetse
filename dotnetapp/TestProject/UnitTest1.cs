...
public class DotnetApplicationTests
{
    ...
    public void Setup()
    {
        ...
        _httpClient.BaseAddress = new Uri("https://8080-abfdabeabcbaedbfedabcebcbbdccdcbecea.premiumproject.examly.io");
    }
    ...
    public async Task Backend_TestRegisterUser()
    {
        ...
        string requestBody = $"{{\"password\": \"abc@123A\", \"userName\": \"{uniqueUsername}\",\"userRole\": \"Student\",\"email\": \"{uniqueEmail}\", \"mobileNumber\": \"9876543210\"}}";
    ...
    public async Task Backend_TestRegisterAdmin()
    {
        ...
        string requestBody = $"{{\"id\": 1, \"password\": \"abc@123A\", \"userName\": \"{uniqueUsername}\",\"userRole\": \"Admin\",\"email\": \"{uniqueEmail}\", \"mobileNumber\": \"9876543210\"}}";
        HttpResponseMessage response = await _httpClient.PostAsync("auth/register", new StringContent(requestBody, Encoding.UTF8, "application/json"));
    ...
    public async Task Backend_TestLoginAdmin()
    {
        ...
        string adminRegistrationRequestBody = $"{{\"password\": \"abc@123A\", \"userName\": \"{uniqueUsername}\",\"userRole\": \"Admin\",\"email\": \"{uniqueEmail}\" ,\"mobileNumber\": \"9876543210\"}}";
    ...
    public async Task Backend_TestLoginUser()
    {
        ...
        string adminRegistrationRequestBody = $"{{\"password\": \"abc@123A\", \"userName\": \"{uniqueUsername}\",\"userRole\": \"Student\",\"email\": \"{uniqueEmail}\",\"mobileNumber\": \"9876543210\"}}";
        HttpResponseMessage registrationResponse = await _httpClient.PostAsync("/auth/register", new StringContent(adminRegistrationRequestBody, Encoding.UTF8, "application/json"));
    ...
    public async Task Backend_TestGetAllAdmissionsAdmin()
    {
        ...
        Assert.IsNotNull(token);
        _httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
        HttpResponseMessage response = await _httpClient.GetAsync("/api/admissions");
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
    }
...
[Test, Order(9)]
    public async Task Backend_TestGetAllStudentCoursesByStudent()
    {
        ...
        Assert.IsNotNull(token);
        _httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + token);
        HttpResponseMessage response = await _httpClient.GetAsync("/api/student/courses");
        Assert.AreEqual(HttpStatusCode.OK, response.StatusCode);
    }
